(function () {
  ...
  class GameRooms {
    ...
    async init () {
      ...
      rooms.map((room) => {
        this.setupRoom(room.id, room.require, this.client)
      })
    }

    async invitePlayers (msg, gamePath, joinReact = '👍') {
    ...
    async createGameRoom (msg, users, name, requirePath) {
    ...
    createGameRoomInhibitor (roomId, gameRequire) {
      return (message) => {
    ...
    setupRoom (roomId, gameRequire, client, first = false) {
    ...
    deleteGameRoom (roomId, client, guild) {
